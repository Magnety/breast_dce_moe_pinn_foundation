...
def _split_by_dataset_id(
    ...
    for idx, sample in enumerate(samples):
        ...
        if not test_set or dataset_id in test_set:
            test_idx.append(idx)
    return {"train": Subset(dataset, train_idx), "test": Subset(dataset, test_idx)}
...
def _split_by_ratio(
    ...
    # Hash on (sample_id, dataset_id, seed) so the assignment is stable across
    # runs and across different machines without requiring numpy.
    train_idx, test_idx = [], []
    for idx, sample in enumerate(samples):
        token = f"{config.seed}|{sample.get('dataset_id', '')}|{sample.get('sample_id', '')}|{sample.get('patient_id', '')}"
