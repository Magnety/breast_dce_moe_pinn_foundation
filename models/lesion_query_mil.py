...
class LesionQueryMIL(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256, num_queries: int = 8, num_heads: int = 8, topk: int = 16) -> None:
        super().__init__()
        ...
        self.pool = nn.Sequential(nn.LayerNorm(embed_dim), nn.Linear(embed_dim, embed_dim), nn.GELU())
